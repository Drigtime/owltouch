...
const resourceList = [];
const resourceNameList = fs.readdirSync(__static + "/resourceLocation/alchemist").map(name => name.split(".")[0]);
resourceNameList.forEach(name => {
  resourceList.push({
    locations: JSON.parse(fs.readFileSync(__static + `/resourceLocation/alchemist/${name}.json`, "utf8")),
    iconUrl: path.join(__static, `/assets/alchemist/${name}.png`),
    name
  });
});
  ...
  createLeafletElement() {
    const resourceContainer = L.Control.extend({
      onAdd: () => {
        ...
        render(<ResourceControl resourceList={resourceList} jobName={"alchemist"} map={this.map} />, this.container);
