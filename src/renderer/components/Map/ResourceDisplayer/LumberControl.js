...
const resourceList = [];
const resourceNameList = fs.readdirSync(__static + "/resourceLocation/lumber").map(name => name.split(".")[0]);
resourceNameList.forEach(name => {
  resourceList.push({
    locations: JSON.parse(fs.readFileSync(__static + `/resourceLocation/lumber/${name}.json`, "utf8")),
    iconUrl: path.join(__static, `/assets/lumber/${name}.png`),
    name
  });
});
  ...
  createLeafletElement() {
    const resourceContainer = L.Control.extend({
      onAdd: () => {
        ...
        render(<ResourceControl resourceList={resourceList} jobName={"lumber"} map={this.map} />, this.container);
