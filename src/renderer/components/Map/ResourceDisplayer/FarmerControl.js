...
const resourceList = [];
const resourceNameList = fs.readdirSync(__static + "/resourceLocation/farmer").map(name => name.split(".")[0]);
resourceNameList.forEach(name => {
  resourceList.push({
    locations: JSON.parse(fs.readFileSync(__static + `/resourceLocation/farmer/${name}.json`, "utf8")),
    iconUrl: path.join(__static, `/assets/farmer/${name}.png`),
    name
  });
});
  ...
  createLeafletElement() {
    const resourceContainer = L.Control.extend({
      onAdd: () => {
        ...
        render(<ResourceControl resourceList={resourceList} jobName={"farmer"} map={this.map} />, this.container);
