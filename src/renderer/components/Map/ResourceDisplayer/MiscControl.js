...
const resourceList = [];
const resourceNameList = fs.readdirSync(__static + "/resourceLocation/miscellaneous").map(name => name.split(".")[0]);
resourceNameList.forEach(name => {
  resourceList.push({
    locations: JSON.parse(fs.readFileSync(__static + `/resourceLocation/miscellaneous/${name}.json`, "utf8")),
    iconUrl: path.join(__static, `/assets/miscellaneous/${name}.png`),
    name
  });
});
  ...
  createLeafletElement() {
    const resourceContainer = L.Control.extend({
      onAdd: () => {
        ...
        render(
          <ResourceControl resourceList={resourceList} jobName={"miscellaneous"} map={this.map} />,
          this.container
        );
