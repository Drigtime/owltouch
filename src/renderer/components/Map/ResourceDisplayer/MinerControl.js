...
const resourceList = [];
const resourceNameList = fs.readdirSync(__static + "/resourceLocation/miner").map(name => name.split(".")[0]);
resourceNameList.forEach(name => {
  resourceList.push({
    locations: JSON.parse(fs.readFileSync(__static + `/resourceLocation/miner/${name}.json`, "utf8")),
    iconUrl: path.join(__static, `/assets/miner/${name}.png`),
    name
  });
});
  ...
  createLeafletElement() {
    const resourceContainer = L.Control.extend({
      onAdd: () => {
        ...
        render(<ResourceControl resourceList={resourceList} jobName={"miner"} map={this.map} />, this.container);
