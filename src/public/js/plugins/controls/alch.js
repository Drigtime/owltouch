...
L.Control.Alchimist = L.Control.extend({
  ...
  onAdd() {
    ...
    this.aButton = L.DomUtil.create('a', 'alch-control dark', container);
    this.aButton.setAttribute('href', '#');
    this.aButton.setAttribute('title', 'Alchimiste');
    this.aButton.style.borderTop = '2px solid #212529';
    this.imgButton = L.DomUtil.create('img', 'no-class', this.aButton);
    this.imgButton.setAttribute('src', './data/assets/alch/alch.png');
...
L.control.alchimist = options => new L.Control.Alchimist(options);
