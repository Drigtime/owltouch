...
L.Control.Alchimist = L.Control.extend({
    ...
    onAdd(map) {
        ...
        this._aButton = L.DomUtil.create("a", "alch-control dark", container);
        ...
        for (let j = 0; j < resourcesLength; j++) {
            json[this.resources[j]] = JSON.parse(fs.readFileSync(path.join(__dirname, `../../../../data/json/alchemist/${this.resources[j]}.json`)));
