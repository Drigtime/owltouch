...
L.Control.Miner = L.Control.extend({
  ...
  onAdd() {
    ...
    this.visible = [];
    this.prof = 'mine';
    this.resources = ['Iron', 'Copper', 'Bronze', 'Cobalt', 'Manganese', 'Tin', 'Silver', 'Bauxite', 'Silicate', 'Dolomite', 'Gold', 'Obsidian'];
    ...
    this.aButton = L.DomUtil.create('a', 'mine-control dark', container);
    this.aButton.setAttribute('href', '#');
    this.aButton.setAttribute('title', 'Mineur');
    this.aButton.style.borderTop = '2px solid #212529';
    this.imgButton = L.DomUtil.create('img', 'no-class', this.aButton);
    this.imgButton.setAttribute('src', './data/assets/mine/mine.png');
    ...
    for (let i = 0; i < resourcesLength; i += 1) {
      ...
      imgMagicButton.setAttribute('src', path.join(__dirname, `../../../../data/assets/mine/${this.resources[i]}.png`));
      L.DomEvent.addListener(aMagicButton, 'click', this.toggle, this);
    }

    for (let j = 0; j < resourcesLength; j += 1) {
      json[this.resources[j]] = JSON.parse(fs.readFileSync(path.join(__dirname, `../../../../data/json/miner/${this.resources[j]}.json`)));
            ...
            L.marker(dofusCoordsToGeoCoords([json[this.resources[j]][key].posX, json[this.resources[j]][key].posY]), {
              icon: L.divIcon({
                ...
                html: `<img src="${path.join(
                  __dirname,
                  `../../../../data/assets/mine/${this.resources[j]}.png`,
                )}"><div class="qnt1">${json[this.resources[j]][key].q}</div>`,
                className: 'mycluster',
                iconAnchor: [
                  sizeOf(path.join(__dirname, `../../../../data/assets/mine/${this.resources[j]}.png`)).width / 2,
                  sizeOf(path.join(__dirname, `../../../../data/assets/mine/${this.resources[j]}.png`)).height / 2,
                ],
              }),
              interactive: false,
...
L.control.miner = options => new L.Control.Miner(options);
