...
L.Control.Info = L.Control.extend({
    ...
    onAdd(map) {
        ...
        // label containers
        this._listContainer = L.DomUtil.create("ul", "list-group uiElement uiHidden dark", container);
    ...
    _update(evt) {
        ...
        if (geoCoords) {
            ...
            for (const key in mapInfo) {
                L.DomUtil.create("li", "list-group-item bg-dark", this._listContainer).innerHTML = `<img src="../data/assets/hint/${
                    mapInfo[key].gfx
                }.png" class="mr-2">${mapInfo[key].nameId}`;
            }
        ...
        for (const iterator of activatedHints) {
            for (const key of Object.keys(this._hints)) {
                for (const child in this._hints[key]) {
                    if (
                        this._hints[key][child].worldMapId === 1 &&
                        (this._hints[key][child].x === x || this._hints[key][child].posX === x) &&
                        (this._hints[key][child].y === y || this._hints[key][child].posY === y) &&
                        this._hints[key][child].categoryId === iterator
                    ) {
