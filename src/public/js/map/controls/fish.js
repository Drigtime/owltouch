...
L.Control.Fisher = L.Control.extend({
  ...
  onAdd() {
    ...
    for (let i = 0; i < resourcesLength; i += 1) {
      ...
      imgMagicButton.setAttribute('src', path.join(__dirname, `../../../../data/assets/fish/${this.resources[i]}.png`));
      L.DomEvent.addListener(aMagicButton, 'click', this.toggle, this);
    }

    for (let j = 0; j < resourcesLength; j += 1) {
      json[this.resources[j]] = JSON.parse(fs.readFileSync(path.join(__dirname, `../../../../data/json/fisher/${this.resources[j]}.json`)));
        ...
        if (!(json[this.resources[j]][key].w === 2)) {
          if (json[this.resources[j]][key].q > 1 && json[this.resources[j]][key].q < 6) {
            L.marker(dofusCoordsToGeoCoords([json[this.resources[j]][key].posX, json[this.resources[j]][key].posY]), {
              icon: L.divIcon({
                iconUrl: path.join(__dirname, `../../../../data/assets/fish/${this.resources[j]}.png`),
                ...
                className: 'mycluster',
                iconAnchor: [
                  sizeOf(path.join(__dirname, `../../../../data/assets/fish/${this.resources[j]}.png`)).width / 2,
                  sizeOf(path.join(__dirname, `../../../../data/assets/fish/${this.resources[j]}.png`)).height / 2,
                ],
              }),
              interactive: false,
