...
$("#defineBankCoordConfirm").click(() => {
  ...
  const marker = L.marker(dofusCoordsToGeoCoords([coord[0], coord[1]]), {
    icon: L.icon({
      ...
      iconAnchor: [sizeOf(`./src/data/assets/path/bank/bank.png`).width / 2, sizeOf(`./src/data/assets/path/bank/bank.png`).height / 2],
      className: "bank"
    }),
    zIndexOffset: 10000,
    interactive: false
...
$("#definePhoenixCoordConfirm").click(() => {
  ...
  const marker = L.marker(dofusCoordsToGeoCoords([coord[0], coord[1]]), {
    icon: L.icon({
      ...
      iconAnchor: [sizeOf(`./src/data/assets/path/phoenix/phoenix.png`).width / 2, sizeOf(`./src/data/assets/path/phoenix/phoenix.png`).height / 2],
      className: "phoenix"
    }),
    zIndexOffset: 10000,
    interactive: false
...
$("#addPutItem").on("click", () => {
  ...
  if (name.length > 0) {
    ...
    $(".delete-item").on("click", e => {
      itemsBank.put.splice(
        itemsBank.put.indexOf(
          parseInt({
...
$("#addGetItem").on("click", () => {
  ...
  if (name.length > 0) {
    ...
    $(".delete-item").on("click", e => {
      itemsBank.get.splice(
        itemsBank.get.indexOf(
          parseInt({
            item: e.target.dataset.id
          })
        ),
