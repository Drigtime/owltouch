...
L.Control.Alchimist = L.Control.extend({
  ...
  onAdd(map) {
    ...
    this._aButton = L.DomUtil.create('a', 'alch-control blue-grey darken-5', container);
    ...
    for (let j = 0; j < resourcesLength; j++) {
      json[this.resources[j]] = require(path.join(__dirname, `../../data/json/alchemist/${this.resources[j]}.json`));
        ...
        if (json[this.resources[j]][key].w === 1) {
          ...
          json[this.resources[j]][key].q < 6
            ...
            dofusCoordsToGeoCoords([
              ...
              icon: L.divIcon({
                ...
                html: `<img src="${path.join(__dirname, `../../data/assets/alch/${this.resources[j]}.png`)}"><div class="qnt1">${json[this.resources[j]][key].q}</div>`,
                className: 'mycluster',
                iconAnchor: [
                  sizeOf(path.join(__dirname, `../../data/assets/alch/${this.resources[j]}.png`)).width /
                  2,
                  sizeOf(path.join(__dirname, `../../data/assets/alch/${this.resources[j]}.png`)).height /
                  2,
                ],
              }),
              interactive: false,
          ...
          json[this.resources[j]][key].q < 11
            ...
            dofusCoordsToGeoCoords([
              ...
              icon: L.divIcon({
                iconUrl: path.join(__dirname, `../../data/assets/alch/${this.resources[j]}.png`),
                html: `<img src="${path.join(__dirname, `../../data/assets/alch/${this.resources[j]}.png`)}"><div class="qnt2">${json[this.resources[j]][key].q}</div>`,
                className: 'mycluster',
                iconAnchor: [
                  sizeOf(path.join(__dirname, `../../data/assets/alch/${this.resources[j]}.png`)).width /
                  2,
            ...
            dofusCoordsToGeoCoords([
              ...
              icon: L.divIcon({
                iconUrl: path.join(__dirname, `../../data/assets/alch/${this.resources[j]}.png`),
                html: `<img src="${path.join(__dirname, `../../data/assets/alch/${this.resources[j]}.png`)}"><div class="qnt3">${json[this.resources[j]][key].q}</div>`,
                className: 'mycluster',
                iconAnchor: [
                  sizeOf(path.join(__dirname, `../../data/assets/alch/${this.resources[j]}.png`)).width /
                  2,
