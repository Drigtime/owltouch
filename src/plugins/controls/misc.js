...
L.Control.Miscellaneous = L.Control.extend({
  ...
  onAdd(map) {
    ...
    this._aButton = L.DomUtil.create('a', 'misc-control blue-grey darken-5', container);
    ...
    for (let j = 0; j < resourcesLength; j++) {
      json[this.resources[j]] = require(path.join(__dirname, `../../data/json/miscellaneous/${this.resources[j]}.json`));
