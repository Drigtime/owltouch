...
L.Control.Farmer = L.Control.extend({
  ...
  onAdd(map) {
    ...
    this._aButton = L.DomUtil.create('a', 'farm-control blue-grey darken-5', container);
    ...
    for (let i = 0; i < resourcesLength; i++) {
      ...
      imgMagicButton.setAttribute(
        'src',
        path.join(__dirname, `../../data/assets/farm/${this.resources[i]}.png`)
      );
      L.DomEvent.addListener(aMagicButton, 'click', this._toggle, this);
    }

    for (let j = 0; j < resourcesLength; j++) {
      json[this.resources[j]] = require(path.join(__dirname, `../../data/json/farmer/${this.resources[j]}.json`));
          ...
          if (
            ...
            L.marker(
              dofusCoordsToGeoCoords([
                ...
                icon: L.divIcon({
                  ...
                  html: `<img src="${path.join(__dirname, `../../data/assets/farm/${this.resources[j]}.png`)}"><div class="qnt1">${json[this.resources[j]][key].q}</div>`,
                  className: 'mycluster',
                  iconAnchor: [
                    sizeOf(path.join(__dirname, `../../data/assets/farm/${this.resources[j]}.png`)).width /
                    2,
                    sizeOf(path.join(__dirname, `../../data/assets/farm/${this.resources[j]}.png`)).height /
                    2,
                  ],
                }),
                interactive: false,
            ...
            L.marker(
              dofusCoordsToGeoCoords([
                ...
                icon: L.divIcon({
                  iconUrl: path.join(__dirname, `../../data/assets/farm/${this.resources[j]}.png`),
                  html: `<img src="${path.join(__dirname, `../../data/assets/farm/${this.resources[j]}.png`)}"><div class="qnt2">${json[this.resources[j]][key].q}</div>`,
                  className: 'mycluster',
                  iconAnchor: [
                    sizeOf(path.join(__dirname, `../../data/assets/farm/${this.resources[j]}.png`)).width /
                    2,
            ...
            L.marker(
              dofusCoordsToGeoCoords([
                ...
                icon: L.divIcon({
                  iconUrl: path.join(__dirname, `../../data/assets/farm/${this.resources[j]}.png`),
                  html: `<img src="${path.join(__dirname, `../../data/assets/farm/${this.resources[j]}.png`)}"><div class="qnt3">${json[this.resources[j]][key].q}</div>`,
                  className: 'mycluster',
                  iconAnchor: [
                    sizeOf(path.join(__dirname, `../../data/assets/farm/${this.resources[j]}.png`)).width /
                    2,
